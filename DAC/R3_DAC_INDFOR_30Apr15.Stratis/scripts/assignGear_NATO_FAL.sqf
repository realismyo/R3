...
- Required Mods: @AGM, @task_force_radio, @HLCMods_Core, @HLCMods_FAL
...
- nul = [this,"loadout"] execVM "scripts\assignGear_NATO_FAL.sqf";
e.g. - nul = [this,"pltld"] execVM "scripts\assignGear_NATO_FAL.sqf";
...
- nul = [this,"loadout",nightGear,scopes,suppressors,"camopattern"] execVM "scripts\assignGear_NATO_FAL.sqf";
e.g. - nul = [this,"pltld",false,true,true,"ctrg"] execVM "scripts\assignGear_NATO_FAL.sqf";
...
- Check the assignGearDefines_NATO_FAL.sqf for defined variables, alter them in that script. 
...
if (count _this > 2) then { if (typeName (_this select 3) == "BOOL") then { _nightGear = _this select 2; }; };
if (count _this > 3) then { if (typeName (_this select 4) == "BOOL") then { _scopes = _this select 3; }; };
if (count _this > 4) then { if (typeName (_this select 5) == "BOOL") then { _suppressors = _this select 4; }; };
if (count _this > 5) then { if (typeName (_this select 6) == "STRING") then { _camoPattern = toLower(_this select 5); }; };
...
#include "assignGearDefines_NATO_FAL.sqf";
