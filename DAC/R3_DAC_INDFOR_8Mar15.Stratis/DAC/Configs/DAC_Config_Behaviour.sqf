...
//-------------------------------------------------------------------------------------------------------------------------
	// Mac defines - always green combat, behaviour safe, speed limited or normal
	case 6:{
		//_setSkill = ["aimingAccuracy","aimingShake","aimingSpeed","Endurance","spotDistance","spotTime","courage","reloadSpeed","commanding","general"];
		_setSkill   = [[0.2,0.5],[0.2,0.8],[0.2,0.8],[0.2,0.8],[0.2,0.6],[0.2,0.8],[0.2,0.5],[0.2,0.8],[0.2,0.8],[0.2,0.8]];
		_setCombat  = ["green"];
		_setBehav   = ["safe","safe","safe","safe","aware"];
		...
		_setJoin    = 2;
		/*_setEmpVeh  = [[150,100],[150,100]];*/
		_setEmpVeh  = [[0,100],[0,100]]; // dont allow boarding of vehicles
		_setSupTime = ["5 + ((skill _unit) * (5 * DAC_AI_Level))",2,5];
		_setHidTime = ["(((10 * DAC_AI_Level) + ((skill _leader) * 50)) / ((count units _group) + 1))"];
	};
//-------------------------------------------------------------------------------------------------------------------------
