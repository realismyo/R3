...
- Updated by Agent, 24Sept2014.
- Removed CA config class check, for usage with A3.
- Removed OA checks, for usage with A3.
*/
...
_removedead = {
	...
	if (isNull _unit || _unit in CLY_dontremovedead || (CLY_removegeared && {_x in CLY_keepgear} count _gear > 0) || (!CLY_removegeared && count _gear > 0)) exitWith {
		CLY_removedeadpending = CLY_removedeadpending - [_unit];
	};
	if (_unit isKindOf "Man") then {
		hideBody _unit;
		_removetime = time + 8;
		while {getPos _unit select 2<0.2 && time<_removetime} do {sleep 0.1};
	};
	CLY_removedeadpending = CLY_removedeadpending - [_unit];
	deleteVehicle _unit;
};
	...
	_alldead = allDead;
	{
		if (isNil {_x getVariable "CLY_removedead"}) then {
			if !(_x in CLY_removedeadpending) then {
				...
				if ((_wait > 0) && ((CLY_removegeared && {_x in CLY_keepgear} count _gear == 0) || (!CLY_removegeared && count _gear == 0))) then {
